...
const MigrationDialog: React.FC<MigrationDialogProps> = ({
    ...
    // Function to extract the relevant error message from a complete error
    const extractRelevantErrorMessage = (errorText: string): string => {
        if (!errorText) return "Unknown error occurred during migration";

        // Look for specific messages and return friendly messages
        ...
        // Extract the last significant line, which usually contains the real error
        ...
        if (lines.length > 0) {
            // Take the last line that usually has the real message
    ...
    // New useEffect to automatically check the Shannon account when there is a selected wallet
    useEffect(() => {
        if (selectedShannonWallet && shannonWallets.length > 0) {
            // Automatically check Shannon account
            verifySelectedShannonAccount();
        }
    }, [selectedShannonWallet, shannonWallets]);

    // Extract the verification logic to a separate function to reuse it
